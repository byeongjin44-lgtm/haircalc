...
1. **진입점 (`/settings`)**: 기존 "기본 설정" 섹션 위에 "사용 설명서" 메뉴 행을
   추가했다(`Link`, 클릭 시 `/settings/guide`로 이동). 기존 설정 섹션 스타일
   (`rounded-2xl bg-white p-5 shadow-sm`)과 동일한 카드 하나로만 추가해 설정 화면
   구조 자체는 건드리지 않았다.
2. **신규 페이지 (`/settings/guide`, 신규 파일)**: 상단 "← 설정" 뒤로가기 링크 +
   "사용 설명서" 제목, 그 아래 10개 섹션(A~J)을 외부 라이브러리 없이 네이티브
   `<details>`/`<summary>` 기반 accordion으로 구성했다(요청받은 "외부 UI 라이브러리
   추가 금지" 원칙 그대로 — 브라우저 기본 토글 동작을 그대로 쓰고 별도 JS state를
   두지 않았다). 각 섹션은 제목 + 1줄 요약 + 펼쳤을 때만 보이는 본문으로 구성해
   긴 벽글을 피했다. 예시는 연한 회색 카드(`ExampleCard`), 정액권 삭제처럼 되돌릴
   수 없는 경고만 빨간 카드(`WarningCard`)로 강조했다.
3. **섹션 구성과 실제 동작 일치 확인**: 작성 전 `engine.ts`(계산 순서: VAT 공제 →
   재료비/카드수수료 공제 → 인센티브율 → 3.3%), `prepaid.ts`(`purchasePrepaidPass`/
   `useOwnPrepaidCredit`/`useByOtherDesigner`/`refundPrepaidCredit`/
   `convertCreditToSalesAmount`), `/settings/page.tsx`(실제 설정 항목), `/settlement/
   page.tsx`(월정산 상단 요약/달력 구조)를 다시 읽고 문서 내용을 그 동작에 맞춰
   작성했다 — A(거래 등록, snapshot 불변 원칙 포함) · B(정산 설정, 실제 존재하는
   6개 항목만) · C(정액권 등록 vs 사용 차이) · D(실결제금액/사용가능금액, 보너스
   빠른 선택) · E(판매 즉시 반영/사용 시 반영, 코드 동작과 일치하는 예시) · F(실결제
   비율 환산/차감금액 기준, 22만원 예시가 기존 테스트값 20만/22만과 정확히 일치) ·
   G(타 디자이너 사용, "개인 정산용"이라는 전제 명시) · H(환불/전액 버튼/조정/삭제,
   삭제는 강조 경고) · I(월정산 요약 항목 + 날짜 클릭) · J(로컬 저장/백업 안내,
   과도하게 겁주지 않는 톤). 횟수형 회원권, 정액권 사용 할인율처럼 아직 구현되지
   않은 기능은 문서에 넣지 않았다.

검증: `npm run lint` 통과(경고 없음) / `npm test` 76개 전부 통과(기존 그대로, 신규
테스트 없음 — 순수 UI/텍스트 추가라 계산 로직 테스트 대상이 아님) / `npm run build`
정상 완료, 빌드 출력에 `/settings/guide`가 "○(Static)"으로 새로 추가된 것을 확인했다.
`git diff --stat`으로 `src/app/settings/page.tsx`(+14줄, 메뉴 카드 1개)와 신규
`src/app/settings/guide/page.tsx` 외에는 아무 파일도 바뀌지 않았음을 확인했다 —
`engine.ts`/`prepaid.ts`/`types.ts`/`recordStore.*`/`migration.ts`/`backup.ts`/
`storage.ts` 전부 무변경. 모바일 360/390/430px 가독성과 실기기 뒤로가기 동작은 이
세션(서버 환경)에서 직접 확인할 수 없어 코드 리뷰(모든 텍스트가 `max-w-md` 컨테이너
안에서 줄바꿈되는 상대 단위/`flex flex-col` 레이아웃임, 별도 고정 너비 없음)로
검증했고, 실기기 최종 확인이 필요하다.
