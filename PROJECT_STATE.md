...
상태: NEXT 1~3 완료, NEXT 4 일부 완료 (정산설정 + 거래등록 + 내역) / 월정산 화면 실제 연결 전

Next.js + TypeScript + Tailwind 프로젝트를 생성했고,
모바일 퍼스트 레이아웃(하단 탭 내비게이션)과
홈 / 거래등록 / 내역 / 월정산 / 설정 5개 라우트를 만들었다.

`src/lib/settlement/types.ts`에 SettlementSettings, Transaction, PrepaidPass,
PrepaidEvent, MonthlyAdjustment 타입을 정의했고,
`src/lib/settlement/engine.ts`에 UI와 완전히 분리된 순수 함수 기반
정산 계산 엔진을 구현했다 (`node --test`로 16개 케이스 검증, 전부 통과).

**"정산 설정 저장 → 일반 거래 입력 → 계산 엔진 적용 → 거래 저장"까지 실제로 동작한다.**
- 정산설정(`/settings`) 화면에서 인센티브율/VAT 방식/카드수수료/재료비/3.3% 설정 후
  `localStorage`에 저장, 새로고침해도 유지됨.
- 거래등록(`/entry`) 화면에서 날짜/금액/고객유형/시술유형/결제수단/메모 입력,
  현재 설정 + 계산 엔진으로 저장 전 예상 정산액을 실시간 표시.
- 저장 시 `buildTransactionSnapshot`으로 적용 인센티브율/VAT 공제액/카드수수료/재료비/
  정산 기준금액/정산액/예상 지급액을 Transaction에 snapshot으로 고정해 `localStorage`에 저장.
  이후 설정을 바꿔도 이미 저장된 거래의 계산 결과는 바뀌지 않음 (직접 검증함).
- 내역(`/history`) 화면에서 저장된 거래 목록을 최신순으로 확인 가능, 새로고침 후에도 유지됨.

아직 하지 않은 것: 홈/월정산 화면을 실제 데이터에 연결, 거래 수정/삭제,
정액권 UI/계산, 월 달력, PWA (다음 작업은 홈·월정산 화면 연결 또는 NEXT 5 정액권).
...
실사용 검증 (2026-09-12):
- 시술금액 110,000원 / 재방문 / VAT 공급가액환산 / 인센티브 40% /
  카드수수료·재료비 없음 / 3.3% OFF → 저장 전 미리보기 정산액 40,000원 확인.
- 실제 UI가 호출하는 것과 동일한 storage.ts/engine.ts 함수로
  "설정 저장 → 새로고침 → 거래 입력 → 저장 → 새로고침 → 내역 확인 →
  설정 변경 후에도 기존 거래 불변" 전체 흐름을 재현해 통과 확인.
- 참고: 이 세션에서는 Claude in Chrome 확장이 연결되어 있지 않아
  실제 브라우저 클릭 조작 테스트는 수행하지 못했다. 위 검증은 UI가 호출하는
  동일 함수를 그대로 실행한 결과이며, 브라우저 수동 확인을 권장한다.
