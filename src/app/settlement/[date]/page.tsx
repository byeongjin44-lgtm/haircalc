...
export default function DailyDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      const [
        ...
        loadedPrepaidEvents,
        loadedPrepaidPasses,
        loadedMembershipEvents,
        loadedMembershipPasses,
      ] = await Promise.all([
        loadTransactions(),
        loadPrepaidEvents(),
        loadPrepaidPasses(),
      ...
      setMembershipPasses(loadedMembershipPasses);
    })();
  }, []);
  ...
  const dayMembershipEvents = filterMembershipEventsByDate(membershipEvents, date).sort((a, b) =>
    a.createdAt.localeCompare(b.createdAt)
  );
  const summary = combineFullPeriodSummary(dayTransactions, dayPrepaidEvents, dayMembershipEvents);
  ...
  function membershipPassLabelOf(event: MembershipEvent): string {
    const pass = membershipPasses.find((p) => p.id === event.membershipPassId);
    return pass?.label || "회원권";
  }

  const hasAny =
    dayTransactions.length > 0 || dayPrepaidEvents.length > 0 || dayMembershipEvents.length > 0;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {dayMembershipEvents.length > 0 && (
        <section className="flex flex-col gap-2">
          <p className="text-sm font-medium text-zinc-500">회원권</p>
          <ul className="flex flex-col gap-2">
            {dayMembershipEvents.map((event) => (
              <li key={event.id} className="rounded-2xl bg-white p-4 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-x-2 gap-y-1">
                  <span className="truncate text-xs text-zinc-500">
                    {membershipPassLabelOf(event)} · {MEMBERSHIP_EVENT_TYPE_LABELS[event.type]}
                  </span>
                  <span className="font-semibold tabular-nums">
                    {formatSignedWon(event.salesImpact)}
                  </span>
